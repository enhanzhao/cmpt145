#a9q3 enz889 Enhan Zhao cmpt145
...
class StudentRecord(object):
    ...
    def drop_lowest(self, grades):
        ...
        for i in range(len(grades)):
            mark = grades[i].scored / grades[i].out_of
            if mark < lowest:
                lowest = mark
                low_obj = i
        section_weight = 0
        for i in grades:
            section_weight += i.weight
        for i in grades:
            i.weight = section_weight / (len(grades) - 1)
        grades[low_obj].weight = 0
...
if __name__ == '__main__':
    ...
    for s in course:
        ...
        s.drop_lowest(s.assignments)    #drops assignment lowest as well
